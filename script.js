...
function loadData() {
    ...
    startTime = savedData.startTime || Date.now(); // Устанавливаем начальное время
    updateUI();
}
...
function earnCurrency() {
    ...
    if (timeSinceLastClick <= 1000) {
        ...
        clicksThisSecond = 1; // Обнуляем при смене секунды
    }
    lastClickTime = currentTime;

    maxCPS = Math.max(maxCPS, clicksThisSecond); // Обновление максимальных кликов в секунду
...
function checkAchievements() {
    Object.keys(achievements).forEach(threshold => {
        if (totalClicks >= parseInt(threshold) && !localStorage.getItem(`achievement-${threshold}`)) {
            ...
            localStorage.setItem(`achievement-${threshold}`, true); // Сохранение достижения
...
function updateStats() {
    ...
    document.getElementById('playTime').textContent = `${playTimeHours}ч ${playTimeMinutes % 60}м ${playTimeSecondsRemainder}с`;
...
// Добавление валюты каждую секунду
...
// Запуск функции добавления в секунду каждую секунду
